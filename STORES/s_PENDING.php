...
$query = "SELECT o.OrderID, o.reference_number, CONCAT(c.FirstName, ' ', c.LastName) AS CustomerName, c.current_location, o.order_date, SUM(oi.quantity * p.Price) as order_total
          ...
          WHERE p.StoreID = '$storeID' AND oi.ostatus = 'pending'
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()) { ?>
                        <tr>
                            ...
                            <td>
                                <form action="process_orderitems.php?action=accept" method="post" style="display:inline;">
                                    <input type="hidden" name="OrderID" value="<?php echo $row['OrderID']; ?>">
                                    <input type="hidden" name="storeID" value="<?php echo $storeID; ?>">
                                    <button type="submit" class="accept-btn">Accept</button>
                                </form>
                                <form action="process_orderitems.php?action=decline" method="post" style="display:inline;">
                                    <input type="hidden" name="OrderID" value="<?php echo $row['OrderID']; ?>">
                                    <input type="hidden" name="storeID" value="<?php echo $storeID; ?>">
